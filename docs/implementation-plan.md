...
| **—** | **任务清单跨轮恢复（M0）** | **✅ M0**（2026-08-23）：fast 跨轮任务板恢复——`task_board` 最近快照未完成项注入【任务板】块（仅 FAST 新消息，L3 前渲染，零新存储）；[task-list-memory spec](./superpowers/specs/2026-08-14-task-list-memory-unification-design.md) · 单测 `TaskBoardRestoreServiceTest`/`ContextMessageBuilderTest` · Live `verify_task_list_restore_live.py`（T1–T4）· **M1–M3 ⬜**（KV Memory 统一 + `kind=todo` / pro 终态导出 / session_search） |
