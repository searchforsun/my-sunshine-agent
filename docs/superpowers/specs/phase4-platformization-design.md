...
| **4.7.9** | **ReAct Request Decision**（`request_decision` 元工具，Cursor 对齐 `questions[]`/`answers[]`；Chat MAIN ✅ / Planner 延后；`enabled` 默认 false） · [详设](./archive/2026-07-28-react-request-decision-design.md) · [cursor-align](./archive/2026-08-11-request-decision-cursor-align-design.md) · Live `verify_decision_live.py` | ✅ Chat MAIN |
