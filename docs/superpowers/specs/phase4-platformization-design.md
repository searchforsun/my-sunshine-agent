...
| **4.8** | 工具集成（SDK + MCP）：MySQL Catalog + `/tools` 管理页 | 业务解耦 / 异构工具接入 | 中 · [详设](archive/2026-07-09-tool-integration-design.md) |
...
**三/四交界（已落地）**：Chat 底栏 **执行路径选择器** P0 ✅（`executionPreference` + `ForcedExecutionRouter`）；**workflow 模板 catalog / `#` 绑定** 归属 **4.13**，见 [chat-execution-mode-selector-design.md](archive/2026-06-25-chat-execution-mode-selector-design.md) §1.1、[workflow-studio-design.md](archive/2026-06-25-workflow-studio-design.md) §3.4。
...
> **详设**：[docs/rag/README.md](../../rag/README.md) · [2026-06-27-rag-knowledge-studio-design.md](archive/2026-06-27-rag-knowledge-studio-design.md) · **缺口** [backlog.md](../../rag/backlog.md)  
...
> **详设**：同上 [rag-knowledge-studio-design.md](archive/2026-06-27-rag-knowledge-studio-design.md) §6.2
...
> **详设 SSOT**：[2026-07-15-skills-docker-sandbox-design.md](archive/2026-07-15-skills-docker-sandbox-design.md) · [方案 B](archive/2026-07-16-conversation-sandbox-permanent-tools-design.md) · [工作区抽屉](archive/2026-07-16-sandbox-workspace-drawer-design.md) · [写确认跳过](archive/2026-07-16-sandbox-write-hitl-skip-design.md)  
...
| **4.5.7** | 可取消沙箱工具（exec/grep/glob）· 主行「已取消」· Live `verify_sandbox_tool_cancel_live` · [详设](archive/2026-07-18-sandbox-tool-cancel-design.md) | ✅ |
...
| **4.6.4** | P5 `ContextCompressor`（AgentScope `AutoContextMemory`） | **✅** · [详设](archive/2026-07-17-autocontext-memory-design.md) |
...
| **4.7.5** | **ReAct TaskBoard**（`manage_tasks` 元工具 + `tasks` Timeline + 审计） · [详设](archive/2026-06-24-react-taskboard-design.md) · **D11** | **✅** |
| **4.7.6** | **ReAct Spawn Subagent**（`spawn_subagent`；卡片+抽屉；**单独取消**） · [详设](archive/2026-07-18-react-spawn-subagent-design.md) · Live `verify_spawn_subagent_live.py` | **✅** |
| **4.7.7** | **ReAct 目标对齐与失败预算**（`GoalAlignmentMiddleware` + `FailureBudgetMiddleware`） · [详设](./archive/2026-07-27-react-goal-alignment-design.md) | ⬜ 候选 |
| **4.7.8** | ~~ReAct Harness Loop 增强~~ · **已归档**（2026-08-10）：阶段四/五负优化；有用残留拆至五层 §4.5 / [goal-alignment §12](./archive/2026-07-27-react-goal-alignment-design.md) / [multi-agent §4.4.1](./2026-07-29-multi-agent-unified-design.md) · [archive](./archive/2026-07-28-harness-loop-enhancement-design.md) | 📦 归档 |
| **4.7.9** | **ReAct Request Decision**（`request_decision` 元工具，Cursor 对齐 `questions[]`/`answers[]`；Chat MAIN ✅ / **Planner D12 ✅**（Planner MAIN 注册 + 暂停/续跑同契约）；Worker/SUB 不注册；`enabled` 默认 false） · [详设](./archive/2026-07-28-react-request-decision-design.md) · [cursor-align](./archive/2026-08-11-request-decision-cursor-align-design.md) · [D12](./archive/2026-08-12-react-request-decision-planner-d12.md) · Live `verify_decision_live.py` | ✅ Chat MAIN + Planner |

**4.7.3 摘要（✅）**：第五顶层模式 `peer-collab`；`ExpertConsultationExecutor` + `ExpertHubEngine`（min/max 轮次、continue、反应式选人）+ `ConsultationSynthesizer`；详设 [expert-consultation-design.md](archive/2026-07-07-expert-consultation-design.md) · Live `verify_peer_collab_live` + `verify_expert_consultation_live`。

### 4.8 工具集成（SDK + MCP）

> **演进 SSOT**：[2026-07-09-tool-integration-design.md](archive/2026-07-09-tool-integration-design.md) · 实施计划：[2026-07-09-tool-integration.md](../plans/2026-07-09-tool-integration.md)  
...
**Phase 1 增量（✅ 2026-07-10）**：Catalog ID `sdk__*` / `mcp__*`（`ToolIds`，无 LLM 转换层）；HITL `require_confirmation`；`execution_mode_policy`；llm-gateway `toolCalls` 日志。详设 [tool-integration-design §6.3 / §9 / §14](archive/2026-07-09-tool-integration-design.md)。
...
**检查门**：`verify_tool_integration_live.py --suite all` — SDK 2 应用 5 工具、ReAct invoke、MCP probe、工具集、HITL、动态 disable；**✅ 已通过**（2026-07-10）。详见 [tool-integration-design §16](archive/2026-07-09-tool-integration-design.md#16-检查门)。
...
> **详设**：[2026-07-20-prompt-ops-routing-catalog-design.md](archive/2026-07-20-prompt-ops-routing-catalog-design.md)  
...
> **详设**：[2026-06-25-workflow-studio-design.md](archive/2026-06-25-workflow-studio-design.md)  
...
**修订（2026-07-11）**：Workflow **DB 唯一 SSOT**；废弃 Nacos `sunshine-workflows.yaml` 与 `Composite*` 合并逻辑。详设 [workflow-studio-design.md](archive/2026-06-25-workflow-studio-design.md) · 计划 [2026-07-11-workflow-studio.md](../plans/2026-07-11-workflow-studio.md)
...
**P0 多 Agent 接入边界**（MsgHub / Parallel / TaskBoard）：[2026-06-25-phase4-agent-capabilities-boundaries.md](archive/2026-06-25-phase4-agent-capabilities-boundaries.md)
...
- [第五模式 Peer 协作路由](archive/2026-06-24-peer-collab-routing-design.md)
- [多 Agent 架构详设](archive/2026-06-19-multi-agent-architecture-design.md) · [锁定决策 D10](archive/2026-06-19-locked-architecture-decisions.md#d10-第五顶层模式--peer_collab阶段四)
