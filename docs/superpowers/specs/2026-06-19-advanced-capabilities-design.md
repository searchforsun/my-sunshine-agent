...
| 失败不自动 replan | 规划期 **Replan**（校验反馈，默认 ≤2）；执行期单节点同参重试；**局部 Replan** 留阶段四 |

### 2.5 执行引擎升级路径

| 阶段 | 能力 | 改动 |
|------|------|------|
| **3.D1** | 线性 Plan 实例化 + 重试/降级 | `PlanWorkflowExecutor` + `NodeRetryExecutor`；Replan；`completed_with_errors` / `degraded_react` |
...
| **C. 双层（推荐）** | Planner 输出 **结构化 Plan JSON** → DAG 执行；MsgHub 收敛为阶段四第五模式 `peer-collab`（[详设](./2026-06-24-peer-collab-routing-design.md) · D10） | 兼顾可控与灵活 |
...
| 并行子 Agent / MsgHub | M10–M11 | 阶段四 **4.7.2–4.7.3** · [peer-collab spec](./2026-06-24-peer-collab-routing-design.md) |
...
| L4 | 并行子 Agent + **`PEER_COLLAB` 第五模式** | 阶段四 4.7.2–4.7.3 · [peer-collab spec](./2026-06-24-peer-collab-routing-design.md) |
