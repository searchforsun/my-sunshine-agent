...
> **v3.3（2026-08-14 · 记忆收敛对齐）**：命名与五层 **v25** / task-scene **v14** 同步——「L2 用户状态」读作 **KV Memory（scope=user）**（§3.1/§3.2/§5）；**换题（清空 sticky seed）协同 [task-list-memory](./archive/2026-08-14-task-list-memory-unification-design.md) 沉淀未完成任务到 KV Memory**（§4.3），随后 seed 清空、目录仍可发现。
...
> **v3.16（2026-08-29 · 运行中技能加载放开 + 常驻 MAIN）**：此前 `sunshine_search_skills`（`SkillSearchTool`）**被候选集白名单卡死**（只允许加载「本轮采纳候选」（`SkillCandidateRegistry.candidates`）内技能）且**仅双阈值采纳开关（Nacos `skill-adoption`）开启时注册**——用户显式 `/` 触发或普通目录技能（如 `brainstorming`/`writing-plans`）根本不在候选集内，且默认开关关闭，导致「运行时加载技能到沙箱工作区」不可用。**放开**：①去掉候选集守卫，改为校验 `enabled` + 租户可见（`TenantVisibility.visible`）；②`DynamicToolkitFactory` 将 `sunshine_search_skills` **常驻 MAIN 注册**（不再依赖 `skill-adoption` 开关）；③保留 `promoteTriggered`（落库触发集 + `mountSkillForBridge` 挂物料到沙箱 `/skills/{id}/`，供 `sandbox__read`/`glob` 读取）。候选集仅保留作目录「可动态加载」提权标记，不再作为本工具的可用性门槛。落点：`SkillSearchTool`（`execute`/描述/注释）· `DynamicToolkitFactory`（注册条件）· `ReactExecutor` 注释。单测 `SkillSearchToolTest` 8 例（新增 `unknownSkill_returnsUnavailable`/`tenantMismatch_returnsNotVisible`）+ `DynamicToolkitFactoryTest`/`AgentInfraTest` stub 适配；orchestrator 全量 1440/1440 全绿。
> **v3.17（2026-08-29 · C1 前缀稳定修正，回退正文到尾部 USER 信封）**：**v3.15 的 SYSTEM 权威层注入与压缩点 C1 前缀稳定冲突**——`SkillInjectionMiddleware.onSystemPrompt` 每次 model call 都把触发集 skill 正文追加进 **system prompt（SYSTEM 前缀，Tier 0 静态核）**，而触发集可被 `sunshine_search_skills` **运行中追加**（v3.16，`appendTriggeredSkillId` → sticky 继承）→ 下一轮 `onSystemPrompt` 生成的前缀 system **逐轮变长** → **C1 前缀稳定被击穿 → 全量 KV 失效**。经反编译核对 AS 2.0（`AgentBase.seedSystemMsg` → `ReActAgent.applySystemPromptMiddlewares`），system 消息**必然位于 messages 前缀**且 `inputMessages` 禁 SYSTEM——「skill 正文在 SYSTEM 权威层」与「不在前缀」在 AS 2.0 下**不可兼得**。**修正（方案C，用户拍板守 C1 优先）**：①移除 `SkillInjectionMiddleware`（不再经 `onSystemPrompt` 注入前缀 system）；②skill 正文信封回退 `PromptComposer.wrapSkillEnvelope`，**置于 `appendReactTail`**（紧邻 user query 之前的 Tier 2 尾部动态段）——触发集中途增长只影响尾部（信封 + 本轮 user query），历史轮次前缀（Far/Mid/Near）不位移，守 C1；③System 层遵循引导由 `system-prompt` **静态【技能遵循】段**（seed 171 v3，字节稳定）承载，不随触发集变化。**已知代价**：skill 正文（HARD-GATE 所在）回到 USER 层（权重低于 SYSTEM），遵循度较 v3.15 回落，仅靠静态 gate 引导——用户已确认接受（C1 优先）。落点：删除 `SkillInjectionMiddleware`/`SkillInjectionMiddlewareTest` · `ProcessingStepMiddlewareFactory`（移除注册与 skillCatalogService 依赖）· `ReActAgentRuntime`（移除 `CTX_TRIGGERED_SKILL_IDS` put）· `PromptComposer.appendReactTail`（注入 skill 信封）。单测 `PromptComposerTest` 更新 23 例；压缩点核心 `ContextAssemblerL1Test`/`L1Compressor*` 全绿。
> **v3.18（2026-08-29 · sunshine_search_skills 完成步走 skill 步骤，统一「加载技能 <技能名>」展示）**：**痛点**——`sunshine_search_skills` 运行时加载技能后，主时间线出现**两处不一致**的「加载技能」：① L0 绑定 skill 走 `phase=skill`（skill 徽章图标 + after=技能名，如「加载技能 brainstorming」）；② `sunshine_search_skills` 工具走 `tool-*`（扳手图标 + after=`{displayName}完成`=「加载技能完成」，工具显示名被 `ToolCatalogService.BUILTIN_TOOL_DISPLAY_NAMES` 设为「加载技能」）→ 用户看到「加载技能 加载技能完成」冗余且图标不一致。**修正（后端契约，SSE 优先）**：让 `sunshine_search_skills` 工具完成步也走 `phase=skill`——`ProcessingStepMiddleware`：①开步（`beginToolStep`）对该工具用 `TimelineStepId.SKILL.id()/phase()`（而非 `tool-sunshine_search_skills`/`tool`），并 `bindStepDisplayName` 为「加载技能」（避免 `skill-{ts}` 动态 id 回退显示）；②收步（`completeToolStep`）拦截 `SkillSearchTool.NAME`，从入参 `skill_id` 取真实技能，`summaryLine=SkillLoadLabels.after(skill_id)`（真实技能名）、`meta=StepMetadata.fromSkillLoad(skill_id)`（`metadata.skillId`）、`expandDetail=工具返回正文`（保留下拉查看技能正文/声明工具）。前端 `TimelineStepIcon` 因 `phase=skill` 自动渲染 skill 徽章图标，label/after 显示「加载技能 <技能名>」，无需改动。落点：`ProcessingStepMiddleware`（开步/收步）+ `ProcessingStepMiddlewareTest`（新增 `onActingSunshineSearchSkillsUsesSkillStep`）。全量 orchestrator 全绿。
> **v3.19（2026-08-29 · 运行中加载技能并入后续轮次触发集，sticky 并集）**：**痛点**——`sunshine_search_skills` 运行中加载技能 X 后（`SkillSearchTool.promoteTriggered` 写入消息 `routing_skill_ids` + 沙箱挂物料），**同轮中断续跑**与**下一轮新消息**注入的仍是**加载前的老技能正文**，而非 X。**根因**：①`SkillSearchTool` 只落库 `routing_skill_ids`，未并入本轮运行时触发集（`AgentRunRequest.triggeredSkillIds` 在意图路由时定格）；②`RoutingStickyService.applySeed` 对 skill 是「triggered 集**空**才继承 seed / **非空**整表替换」——续跑/下一轮 plan 常因 L1 规则带老技能触发集（非空），seed 里动态加载的 X 被整表替换掉。**修正（S-1 语义修正，符合 spec §3.1/§4.6「加载即升级本轮 triggered → 后续 sticky 继承」）**：`RoutingStickyService.applySeed` 对 skill 改为——**非 L0 显式绑定**的 plan，seed 技能与 plan 已触发集做**并集**（`union(plan.triggered, seed)`，保序去重，`PARAM_SKILL` 补首项）；**仅 L0 显式 `/skill`**（`reason` 以 `skill:` 开头 = `skill:/mention|hint|client`）保留**整表替换**语义（用户显式指定新技能覆盖旧触发）。这样续跑/下一轮都能经 seed 并集把 X 并入触发集 → `PromptComposer` 注入 X 正文。**关于方案 A（同 run 内运行时触发集宿主）**：经核对 AS 2.0 ReAct 主链 `composeReactInputs` 仅在 `run` 入口一次性组装 `inputs`（skill 正文信封在 `appendReactTail` 尾部），先于 `sunshine_search_skills` 工具执行，故**同 run 内无法再注入新 skill 正文**；但 `sunshine_search_skills` 返回的正文本身经 `tool_result` 已进本轮上下文（模型同轮可见），无信息损失。故不做运行时宿主，以 seed 并集覆盖续跑/下一轮。落点：`RoutingStickyService` + `RoutingStickyServiceTest`（新增 `applySeed_l1Trigger_mergesRuntimeLoadedSeed`）。全量 routing 106 例 + orchestrator 全绿。
> **v3.20（2026-08-29 · 「加载技能」步骤仅渲染动态加载动作，意图绑定/继承不渲染）**：**痛点**——同一会话时间线出现**过剩/重复**的「加载技能」步骤：① 意图绑定（L0 `/skill`、L1 规则触发）与 **sticky 继承**的技能被 `ChatStreamExecutor` 在意图完成后渲染为「加载技能 <技能名>」，导致**继承轮**（无新动态加载）也重复显示继承的老技能（如截图下一轮只见「加载技能 brainstorming」，而动态加载的 writing-plans 未显示、被误当作加载动作的是继承的老技能）；② 同一轮 L0 绑定 + 动态加载时显示两条「加载技能」（brainstorming + writing-plans），冗余。**语义对齐（用户确认）**：动态加载与意图绑定/继承**没有区别，只有「当前有效」**——按加载顺序累积触发集（`routing_skill_ids` 并集，v3.19 保留），但「加载技能」时间线步骤**只在真正执行 `sunshine_search_skills` 动态加载动作时渲染**；意图绑定与 sticky 继承只是「触发/继承」，不算「加载动作」，**不渲染**。**修正**：`ChatStreamExecutor` 删除意图完成后对 `plan.params().get(PARAM_SKILL)`（单数代表技能，意图绑定/继承）的 `completeSkillLoad` 调用——「加载技能」步骤仅由 `ProcessingStepMiddleware` 工具完成步（`sunshine_search_skills` 动态加载）渲染，且 `SkillSearchTool` 已完成步带真实技能名（v3.18）。这样：动态加载轮显示「加载技能 <技能名>」，继承轮/意图绑定轮不显示；子 agent 抽屉 skill 步骤（`SpawnSubagentTool.skillLoadToken`，subSteps）不属主时间线，保留。落点：`ChatStreamExecutor`（删除 `completeSkillLoad` 调用）。单测 `ChatStreamExecutor` 相关 + 全量 orchestrator 1437/1437 全绿。**前端展示（2026-08-29）**：动态加载的「加载技能」步骤（`skill@{ts}`，带真实时间戳）原本被 `OperationStack.isPinnedHeaderPhase` 当作**头部钉扎步**（强制排 `intent → skill → tasks`，即使时间戳乱序也顶到意图后），导致它被固定在意图之后、而非它真正执行的位置（如 `think-5` 后）。修正：`isPinnedHeaderPhase`/`pinnedHeaderOrder` 移除 `skill`（只留 intent/tasks），并把 `phase=skill` 加入 `isTimelineStickyStep`（按真实时序始终露出、不被 roundGroup 折叠吞掉）。这样「加载技能」出现在它真正执行的位置。落点：`OperationStack.vue`。前端 vue-tsc 类型 + 全量 322 单测全绿。
> **v3.21（2026-08-29 · L0 首次绑定统一走加载入口，完整正文统一 + 首现渲染）**：**痛点**——v3.20 把「加载技能」步骤限定为**仅 `sunshine_search_skills` 动态加载**渲染，导致 **L0 首次绑定**（如 `/brainstorming`）的技能在时间线上**无任何加载动作展示**（静默注入），且其正文仅 `systemOverlay`（无声明工具），与动态加载的「# 技能·名 + 正文 + 声明工具」完整正文**格式分叉**；用户动态加载 writing-plans 后下一轮只见「加载技能 brainstorming」而 writing-plans 未显示，且工具/继承渲染混乱。**用户确认语义**：平台自动调 `sunshine_search_skills`（把完整正文注入 USER skill 尾部 + 渲染「加载技能」步骤）与模型主动调用**等价**——正文进上下文 + 前端统一展示层面一致。因此 L0 首次绑定也走该入口，**继承/重复不重复显示**。**修正**：① **正文统一**——新增 `SkillBodyRenderer`（提取 `SkillSearchTool.renderLoadedSkill`），`PromptComposer.resolveSkillOverlays` 对**所有触发集技能**（L0/动态/继承）统一渲染完整正文（# 技能·名 + 正文 + 声明工具 schema，声明工具 ⊆ (tenant,kind) 集、越界标注不可用），替代原 `overlayOrEmpty`（仅 systemOverlay），`SkillSearchTool` 亦委托 `SkillBodyRenderer`；`overlayOrEmpty` 死代码删除。② **首现渲染**——`ChatStreamExecutor` 意图完成后，对 `plan.triggeredSkillIds()` 中**不在** `ctx.routingSeed().skillIds()`（上轮已触发集）的技能渲染「加载技能」步骤（`completeSkillLoad(skillId)`，真实时间戳）；继承技能（在 seed）与动态加载（`ProcessingStepMiddleware` 工具完成步）不重复渲染。这样：L0 首次绑定显示「加载技能 <技能名>」+ 完整正文；继承轮不重复显示；格式全链路统一。落点：`SkillBodyRenderer`（新）· `SkillSearchTool`（委托 `SkillBodyRenderer`）· `PromptComposer.resolveSkillOverlays`（用 `SkillBodyRenderer.renderById`）· `ChatStreamExecutor`（首次种子判定渲染 `completeSkillLoad`）。单测 `SkillBodyRendererTest` 5 例 + `PromptComposerTest`/`SkillSearchToolTest`/`SkillCatalogServiceTest` 全部改 mock 适配 + 全量 orchestrator 全绿（截止全部通过）。⏳ v3.6 retrieval 双层；[spec](docs/superpowers/specs/2026-08-12-skill-sticky-process-chain-design.md)
...
> **v3.23（2026-09-04 · 目录与触发集解耦，守 C1 前缀字节稳定）**：**痛点**——`.context.skill-directory` 可发现目录层位于 Prompt 前缀稳定区（`appendCommonReactLayers` 在 scope-prompt 之前的 `addReactUser(skillDirectory)`），但 `discoverableForPrompt` 按**剔除已触发项**过滤——用户加载新技能（L0 `/`、sticky 继承、运行中 `sunshine_search_skills` 升级 triggered）后，触发集变化 → 目录**少一行**，其后整个前缀（目录 + scope + 上下文层 + 历史轮次）字节位移 → **厂商前缀 KV cache 全量失效**，`cached_tokens` 骤降（命中率掉到个位数）。**根因**：前缀字节稳定被"目录随触发集内容变化"击穿，与 spec §8「目录 == 字节稳定」要求矛盾。**修正（守 C1，方案沿用 v3.17 的尾部承载触发态）**：① `discoverableForPrompt` **不再剔除已触发项**——目录条目仅由 `(enabled, kind, tenant)` 决定，与触发集完全解耦，目录字节恒定；② **"已触发/已加载"状态不回前缀**，由尾部 skill 正文信封 `<skills_referenced>` 索引承载（v3.14，随触发集变化只影响尾部动态段）；③ 目录 javadoc + PromptComposer `resolveSkillDirectory` javadoc 同步更新（去掉"剔除已触发项"表述）。**已知代价**：目录仍会列出已触发的技能（同名+描述），模型可从 `<skills_referenced>`（尾部）区分哪些已生效，重复列出无功能影响；候选集（S-C）提权置顶仍保留，`skill-adoption` 关闭时恒空不影响字节稳定，开启后候选提权会随候选集变化（属 S-C 留待下一阶段，不在此处收口）。**落点**：`SkillCatalogService.discoverableForPrompt`（去剔除触发项 + javadoc）· `PromptComposer.resolveSkillDirectory`（javadoc）。单测 `SkillCatalogServiceTest.discoverableForPrompt_filtersEnabledAndKind_keepsTriggeredForStablePrefix` + `PromptComposerTest` 全绿。
> **v3.24（2026-09-12 · 目录去候选集提权，恢复逐消息字节稳定）**：**痛点**——v3.23 收口了「目录随触发集剔除」的击穿，但 S-C **候选集提权**仍是目录层最后一个逐消息变量：`renderDiscoverableForPrompt` 按每条消息的 L3 候选集 `promoteCandidates` 置顶重排 + 追加「（可动态加载）」后缀——目录位于全部历史轮次之前（前缀稳定区），候选集逐消息变化 → 每发一条新消息**全部历史前缀重 prefill**，加载新技能场景下前缀缓存照旧全量击穿。且 v3.16 已将 `sunshine_search_skills` 放开为「任意 enabled 技能」，候选集仅剩这一个装饰性用途，`SkillCandidateRegistry` 无读取方。**修正（前缀稳定不变量收口）**：① `renderDiscoverableForPrompt` 删除候选提权与「可动态加载」标记，签名收敛为 `(sessionKind, topN, tenantId)`——目录输出仅由 `(enabled, kind, tenant, 技能目录本身)` 决定，逐消息字节稳定；② **候选集全链路退役（无残渣）**——`SkillCandidateRegistry`（删除）· `ReactExecutor`（`PARAM_CANDIDATE_SKILL_IDS`/`resolveCandidateSkillIds`/registry bind）· `AgentRunRequest`（`candidateSkillIds` 组件 + `withCandidateSkillIds`）· `PromptComposeRequest`（组件 + S-C 工厂重载）· `ExecutionPlan`（param 常量 + `candidateSkillIds()`）· `SkillAdoptionService`（候选档产出，保留 trigger 触发 + agent 可调度池）· `AgentExecutionProperties.SkillAdoption.candidateTopK`；未触发技能由模型经目录 + `sunshine_search_skills` 按需加载（v3.16 语义不变）。③ **联动 L2 后置**（见统一上下文压缩 spec §5.5.3 v26）：`ContextMessageBuilder` 把 `l2SystemBlock` 从前缀区（layer-prompt 合并条）后置到 TaskListRestore 之后、L3 之前——L2 由每轮写路径静默更新且条目带 TTL，置于历史之前会让任一次写入/过期击穿其后全部历史；后置后与任务板恢复块同级，击穿半径收敛到尾部动态段。**落点**：`SkillCatalogService` · `PromptComposer` · `SkillAdoptionService` · `ExecutionPlan` · `AgentExecutionProperties` · `AgentRunRequest` · `PromptComposeRequest` · `ReactExecutor` · `ReActAgentRuntime` · `ContextMessageBuilder`；Nacos `skill-adoption` 去 `candidate-top-k`。单测：`SkillCatalogServiceTest`/`SkillAdoptionServiceTest`（去候选档断言）/`PromptComposerTest`/`ContextMessageBuilderTest`（顺序断言 LayerPrompt→Far→Mid→Near→TaskList→L2→L3）适配；orchestrator 全量全绿。
...
对齐 [task-scene §7](./archive/2026-08-01-task-scene-context-design.md)：目录摘要稳定前缀；命中正文进动态段 / skill-overlay。
...
退出技能 / 明确换题 → 清空 seed（同时把未完成任务沉淀到 KV Memory，见 [task-list-memory §6](./archive/2026-08-14-task-list-memory-unification-design.md)）
...
| 风险 | 对策 |
|------|------|
| 目录过长占前缀 | Top-N + 「更多经 / 或检索」；对齐 task-scene 字节稳定（v3.23：目录与触发集解耦——不剔除已触发项，触发态由尾部 `<skills_referenced>` 承载；候选提权置顶随 S-C 开启才生效，属留待下一阶段） |
...
| [business-context-authority](./archive/2026-08-13-business-context-authority-design.md) | 触发稳定有助于 biz_scene；可发现≠ scene 乱跳 |
