...
**节点 ID**：业务节点 `{type}-{8位hex}`（种子与 Studio 新建一致）；`start` / `answer` 固定 id。下游 `{{node-id.*}}` 引用须与 id 一致。

### 4.3 节点 params 与项目配置对齐

| type | params 关键字段 | 配置来源 |
|------|----------------|----------|
| **rag** | `query`（必填）、`context`（可选）、`topK`、`kbId` | Studio 分组面板；默认见 `sunshine-workflow-manager.yaml` `node-param-defaults` |
| **tool** | `tool`（Catalog ID）、业务入参、`output.mode` / `output.extract`（可选） | `GET /api/tools/catalog` schema |
| **agent** | `skill`、`query`、`context`、`tools[]`、`maxIters`、`systemOverlay`、`kbId` | skills + tools catalog |
| **answer** | `prompt`（含 `{{node-id.output}}` / `{{node-id.answer}}`） | Studio 文本编辑 |
| **join** | （无业务 params）+ `retry.*` | 4.7.2 并行汇总 |
| **重试（可选）** | `retry.maxAttempts`、`retry.backoffMs`、`retry.onFailure` | Studio「执行策略」分组；全局默认见 `/tools` `execution_mode_policy` |
...
`start` / `rag` / `tool` / `agent` / `answer` — 与 `WorkflowNodeType` 一致。高级图另含 `join`（4.7.2，种子 `knowledge-dual` 已覆盖）。

### 7.1.1 业务节点配置面板（分组）

右侧面板按节点类型分组（`WorkflowNodeConfigSection`），保持 `--sun-black` + 边框卡片、组内统一 `gap`：

| 节点 | 分组 |
|------|------|
| **rag** | 基本信息 → 输入（query/context）→ 检索配置（topK/知识库）→ 输出 → 执行策略 |
