...
- 读取本包 `scripts/`、`references/`（容器内挂载为只读 `/skill`）
- 在可写 `/workspace` 生成或修改文件后执行命令

## 操作步骤（Agent）

1. 用 `sandbox__glob` 或 `sandbox__read` 查看 `/skill/scripts`、`/skill/references`
2. 需要改文件时：只写 `/workspace/...`（禁止写 `/skill`）
...
```text
@sandbox-coding-demo 请用沙箱工具：读取 /skill 下脚本，在 /workspace 写 test.txt，再 ls
```
...
- 禁止越狱路径（`/tmp`、`..` 逃逸等）
- 默认无外网；需要 pip 等外连时由管理员配置 `networkAllow`
