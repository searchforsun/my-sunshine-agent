...
class PromptRoutingSupportTest {
    ...
    void validateDetectsSamePriority() {
        List<RoutingRuleDef> rules = List.of(
                routingSupport.parse("routing-rule.a", 20, true, FINANCE_LIST_JSON),
                routingSupport.parse("routing-rule.b", 20, true, FINANCE_LIST_JSON));
    ...
    private List<RoutingRuleDef> t0Rules() {
        return List.of(
                ...
                routingSupport.parse("routing-rule.peer-phrase", 90, true, PEER_JSON),
    ...
    private void stubT0DbRules() {
        when(definitionRepository.findByKindAndEnabled(PromptRoutingSupport.ROUTING_RULE_KIND, true))
                .thenReturn(List.of(
                        ...
                        routingDef("routing-rule.peer-phrase", 90),
        ...
        when(versionRepository.findByPromptIdAndVersion("routing-rule.peer-phrase", 1))
                .thenReturn(Optional.of(published("routing-rule.peer-phrase", PEER_JSON)));
