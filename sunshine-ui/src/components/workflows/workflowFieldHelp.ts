...
const NODE_HELP: Record<string, string> = {
  ...
  toolOutputMode: '选择下游默认引用的输出形态：完整原文、Catalog 摘要模板，或自定义 regex/json/line 提取。',
  toolOutputExtract: '提取规则 JSON：值支持 regex:<pattern>（第 1 捕获组）、json:<path>、line:<n>（0 起行号）。与 /tools 时间线摘要提取语法一致。',
