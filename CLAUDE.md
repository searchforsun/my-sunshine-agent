...
| 新 Workflow | **4.13**：`/workflows` + `workflow-manager` DB（**唯一 SSOT**，废弃 Nacos workflow）；MySQL init 种子 **5 标杆**（`docs/workflow/` ↔ `13-sunshine-workflow-manager.sql` 同构）；orchestrator `WorkflowManagerClient` |
