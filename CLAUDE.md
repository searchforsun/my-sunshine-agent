...
**JVM 内存参数**：服务启动内存由 `sunshine_lib.py` 的 `JVM_HEAP_BY_SERVICE` + `jvm_memory_opts()` 控制（`-Xms/-Xmx/-XX:MaxMetaspaceSize/-XX:MaxDirectMemorySize/-XX:+UseG1GC`），**禁止在 `start_java_detached` 里省略堆配置**（否则 JVM 默认预保留 1/4 物理内存导致 RSS 虚胖）。`orchestrator` 为核心编排（多智能体/Planner-Executor/上下文压缩），堆最大 **2G**；新增服务需在此映射表登记堆大小，默认兜底 1G。改 `JVM_HEAP_BY_SERVICE` 后需重启对应服务生效（`scripts/start.py --restart`）。
