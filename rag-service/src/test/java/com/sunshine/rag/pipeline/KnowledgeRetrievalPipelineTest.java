...
class KnowledgeRetrievalPipelineTest {
    ...
    void setUp() {
        ...
        pipeline = new KnowledgeRetrievalPipeline(retrievalService, queryRewritePipeline, props);
    }
    ...
    void searchReturnsFirstHitsWithoutEmptyRecall() {
        ...
        when(retrievalService.search("q", 3, "hybrid+rerank", "default")).thenReturn(Mono.just(hits));
    ...
    void searchAppliesRagRewriteBeforeRetrieval() {
        ...
        when(retrievalService.search("公司报销管理制度 差旅报销", 3, "hybrid+rerank", "default"))
    ...
    void searchRetriesWhenEmptyAndRewriteEnabled() {
        ...
        when(retrievalService.search(eq("口语问"), anyInt(), anyString(), eq("default")))
    ...
    void searchUsesHydeAsFallbackAfterFirstSearchEmpty() {
        ...
        when(retrievalService.search("公司差旅费报销管理办法", 3, "hybrid+rerank", "default"))
                .thenReturn(Mono.just(List.of()));
        when(queryRewritePipeline.hydeForRag("报差旅"))
                .thenReturn(QueryRewriteOutcome.of("hyde", "报差旅", "员工出差应提交差旅审批单并附发票", 2L));
        when(retrievalService.search("员工出差应提交差旅审批单并附发票", 3, "hybrid+rerank", "default"))
    ...
    void searchIncludesTraceWhenRequested() {
        ...
        when(retrievalService.search("优化q", 3, "hybrid+rerank", "default"))
                .thenReturn(Mono.just(List.of()));
        when(queryRewritePipeline.isHydeEnabled()).thenReturn(false);
