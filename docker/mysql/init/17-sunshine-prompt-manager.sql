...
- **中间正文（过渡）**：仍需调 tool 时，发起业务 tool call **前必须**先调用 `think_summary` 工具输出本轮摘要（20字以内）；content 再输出 **1–2 句**进展/下一步过渡语；禁止表格、分节、合规长文、**结论**、**汇总**、对用户问题的**完整作答**。
- **终态正文（唯一完整答复）**：仅在最后一轮、确认不再调用任何 tool 后，先调用 `think_summary` 工具输出 `summary=完整回答用户问题`，content 再 **一次性完整回答**用户全部子问题（查询结果、合规分析、能否提交审批、建议等）；此前各轮勿提前给出结论或终局答案。
- 【think_summary·规则】每轮发起业务 tool call 前，**必须先调用** `think_summary` 工具，`summary` 参数填本轮 20 字以内摘要（如「先规划以下要做的事」「分析检索到的政策文件」）；平台将其展示为时间线步骤标题，摘要文本**不要**写入 content。
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.intent', 1, 'published', NULL, '{"label":"识别意图","before":"识别用户意图","active":"正在匹配最佳处理方式","default-after":"已完成意图判断","unmatched-after":"将按「{detail}」处理","modes":{"react":{"detail":"自主智能体","after":"将由自主智能体分析并作答","forced-after":"将按您指定的「自主推理」模式处理"},"plan-workflow":{"detail":"动态规划","after":"将动态规划多步执行","forced-after":"将按您指定的「动态规划」模式处理"},"peer-collab":{"detail":"多专家协作","after":"将由多专家协作交叉验证","forced-after":"将按您指定的「多专家协作」模式处理"}}}', 'nacos migrate', 'migrate_nacos_prompts_to_db');
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.steps.think', 1, 'published', NULL, '{"label":"深度思考","before":"规划如何回答{query}","active":"正在规划{query}的工具调用方案","after":"已完成{query}的工具调用规划","before-fallback":"规划工具与作答路径","active-fallback":"正在规划工具调用方案","after-fallback":"工具调用方案已拟定","before-follow-up":"准备结合{toolDisplayName}结果继续分析","active-follow-up":"正在综合分析{toolDisplayName}返回结果","after-follow-up":"已完成{toolDisplayName}的工具结果综合分析","before-follow-up-no-tool":"准备结合工具结果分析{query}","active-follow-up-no-tool":"正在结合工具返回结果分析{query}","after-follow-up-no-tool":"工具结果综合分析已完成","before-follow-up-fallback":"准备结合工具结果分析","active-follow-up-fallback":"正在综合分析工具结果","after-follow-up-fallback":"工具结果分析完成"}', 'nacos migrate', 'migrate_nacos_prompts_to_db');
...
- 无法提取的字段填空字符串
- 只输出 JSON，不要多余解释
- 输出格式：{"field1":"value1","field2":"value2"}', NULL, 'wf-4 ParameterExtractor node template', 'prompt-ops');
