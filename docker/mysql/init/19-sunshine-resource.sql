...
INSERT IGNORE INTO prompt_definition (id, kind, display_name, description, enabled, priority, active_version, catalog_version) VALUES ('mode-overlay.react', 'mode-overlay', '模式覆盖 · ReAct', 'ReAct 模式叠加层：约束自主推理时如何选工具、写思考与最终作答。', 1, 0, 2, 1);
...
- 临时子 Agent：也可不传 agent_id，仅传自包含 prompt（沿用主 Agent 工具集）。', NULL, '硬编码提示词迁移', 'agent');
...
UPDATE prompt_definition SET active_version = 2, catalog_version = catalog_version + 1 WHERE id = 'mode-overlay.react';
UPDATE prompt_catalog_meta SET catalog_version = 72, updated_at = CURRENT_TIMESTAMP WHERE id = 1;