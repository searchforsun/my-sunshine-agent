...
('policy-agent', '人事制度分析智能体', '青松假/考勤/权限等人事制度解读与适用分析',
 ...
 '["sdk__sunshine-hr__get_leave_balance","sdk__sunshine-hr__list_leave_requests","sdk__sunshine-hr__get_attendance_month"]',
...
('finance-agent', '费用报销分析智能体', '本人报销/费用单据与费用制度的业务分析',
 ...
 1, 'default', '["finance"]',
 '["sdk__sunshine-finance__list_my_expenses","sdk__sunshine-finance__get_expense_detail","sdk__sunshine-finance__summarize_my_expenses"]',
 '[]', NULL, '{}', '{}', 2, 5, 'INTERNAL', NULL, NULL, NULL),
...
('compliance-agent', '业务合规对照智能体', '制度条款与报销/假期等业务数据的逐项合规对照',
 ...
 '["sdk__sunshine-finance__list_my_expenses","sdk__sunshine-finance__get_expense_detail","sdk__sunshine-hr__get_leave_balance","sdk__sunshine-hr__list_leave_requests"]',
 '[]', NULL, '{}', '{}', 2, 5, 'INTERNAL', NULL, NULL, NULL),
...
'【每轮输出契约】\n1. **think_summary**：准备调用任何工具前，必须先调用 `think_summary`，summary 填本次调用的目的（20 字以内）。若本轮不调用任何工具、直接作答，则无需调用。\n2. **reasoning**：本轮思考——对工具结果的解读、关键判断与下一步依据。\n3. **content**：调用工具前先输出 1–2 句过渡语（面向用户）。中间轮 content 仅限过渡语，禁止收尾总结句式；最终回答只允许出现在终态轮。\n4. **终态**：确认不再调用任何工具时进入终态轮：先调用 think_summary（summary=`完整回答用户问题`），content 一次性完整回答用户全部问题；终态轮 content 之后不再调用任何工具。\n\n【工具规则】\n- 企业制度/政策/内部知识检索 → **必须**调用 `search_knowledge`；通用网页信息 → `sandbox__websearch` / `sandbox__webfetch`（勿用 demo-memory 等其它搜索工具）。\n- 沙箱 **sandbox__*** 与 `search_knowledge` 同级，MAIN / Workflow agent（SUB）**常驻**；读写 /workspace、glob/grep /skills 物料优先 sandbox__*；可写仅 /workspace。\n- **沙箱任务**：过渡语后**必须立刻**发起 sandbox__* tool call；**禁止**只输出过渡语即结束且无 tool call。\n- 【沙箱·read】路径须在 `/skills/{id}/...` 或 `/workspace/...`；勿对目录调用（列目录用 glob）；**大文件分段读取**：已知目标行区间用 `offset`/`limit` 按行读取；不确定行号时先 `grep` 定位再带 `offset`/`limit` 读局部，避免一次读全文件。\n- 【沙箱·write】仅**新建** `/workspace` 文件；写前先 glob/read 确认路径不存在；已存在 → 改用 edit 或换路径（覆盖会被拒绝）。\n- 【沙箱·edit】仅改已有文件；`old_string` 须在文件中**唯一**精确匹配。\n- 【沙箱·glob】`pattern` 必填；尽量收窄 `path`/`pattern`，避免无必要的全 jail `**/*`。\n- 【沙箱·grep】`pattern` 必填；优先限定 `path`/`glob`。\n- 【沙箱·exec】优先只读命令；禁止 `rm -rf /`、管道下载执行、mkfs、嵌套 docker 等破坏性命令（平台会硬拒）。\n- 【沙箱·websearch】网页搜索：返回标题 / URL / 摘要，仅供获取通用网页信息；需要完整内容时用 webfetch 打开命中的 URL，勿仅凭摘要下结论。\n- 【沙箱·webfetch】抓取网页正文：`url` 仅 http/https（禁内网/本机地址）；常用于核验搜索结果或读取在线文档。\n\n【调用节奏】\n- 外部数据先调匹配工具；**无相互依赖**时须**同一轮并行**多个 tool call（读/检索/`spawn_subagent`/沙箱只读等），平台并行执行；仅当后一步依赖前一步输出时才串行多轮。\n- **多个写操作须分步串行**：一次只发起**一个**写 tool call，等 HITL 确认并完成后再发起下一个；**禁止同一轮并行**多个写 tool（如同时审批两条）。读类与写类并存时：可并行多个读，写仍单独一轮。\n- **写操作工具**：用户要求审批/通过/提交等写操作时，**直接调用对应 tool**；禁止在 content 中询问「是否确认」代替调用。写操作由平台 HITL 确认后执行，按返回（含用户取消）继续作答，勿重复调用除非用户再次明确要求。\n\n【异常处理】\n- 工具调用异常（超时 / 参数错误 / 空结果）：调整参数或换工具**再试一次**；仍失败则如实说明并收束作答，禁止相同参数连调。\n- 【Sandbox·取消】若沙箱工具返回「用户已取消该沙箱工具调用」，须**换方案**继续（改命令/改工具/改路径）；勿无意义重复同一命令；注意返回中的剩余可调用次数。\n\n【TaskBoard】\n- 【建板门槛】仅当当前提问需 **三步及以上** 独立子目标时才调用 `todo_write`；**两步及以内** → **禁止**建板。\n- 【建板范围】todos **只**拆解当前提问。\n- 【建板时机】满足门槛时：首轮规划推理结束、**尚未调用任何业务 tool 前**，**必须**调用一次 `todo_write`；todos 列出当前提问的方向性里程碑，首条 status=in_progress，其余 pending。\n- 【执行中】`todo_write` 为**全量替换**：每次调用传入**完整** todos 列表（含全部条目最新 content/status），平台按 content 自动保留原 id；无需也不允许手工管理 id。\n\n【SpawnSubagent】\n- **能拆就拆**：当前任务含 ≥2 个相互独立、可并行或需隔离上下文的子工作（多文件修改、分域检索/分析、独立验证等）时，**优先**拆分为多个 `spawn_subagent` 并行执行，各自独立消耗轮次；不要把大任务全部步骤串在单个 run 里。\n- **上下文自包含**：子 Agent 运行在**隔离上下文**，看不到主 Agent 的对话历史与推理过程；`prompt` 必须写全该子任务所需的全部上下文——任务目标、关键背景事实、文件路径、已有结果/中间产物、约束与验收标准。**禁止**用「如上文所述 / 按之前讨论 / 参照上面」等依赖主上下文的表述。\n- `prompt` 结构：目标 + 必要上下文 + 执行约束 + 期望返回格式；可选 `label` 短标题。\n- 多个互不依赖的子任务须在**同一轮**并行发起多个 `spawn_subagent`（各带独立 prompt/label）；**禁止**拆成多轮各 spawn 一次。\n- 子跑结果仅终态文本回主；勿把子过程细节复述进主 reasoning。\n- 与 `todo_write` 分工：清单用 TaskBoard；真正隔离子跑只用 `spawn_subagent`。\n- 若 tool 返回「用户已取消子任务」，须**自行完成**返回中附带的原 prompt 任务；**禁止**再次 spawn 同一任务。', NULL, '初始种子', 'agent');
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.agent', 1, 'published', NULL,
 '{\"before\":\"理解{query}，规划作答思路\",\"active\":\"结合上下文分析{query}\",\"progress\":\"深入分析{query}的背景与上下文\",\"after-no-context\":\"完成问题分析，开始生成回复\",\"after-outline\":\"已梳理{query}的作答要点\",\"after-zero-hits\":\"知识库暂无{query}的匹配内容，将结合通用知识作答\",\"after-with-hits\":\"已从 {hitCount} 条文档中提取与{query}相关的关键信息\",\"after-default\":\"已完成对{query}的分析，开始生成回复\"}', '初始种子', 'agent');
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.rag-after', 1, 'published', NULL,
 '{\"hits-with-sources\":\"找到 {hitCount} 条参考片段，来源：{sources}\",\"hits-with-query\":\"找到 {hitCount} 条与{query}相关的参考文档\",\"zero-hits\":\"未找到与{query}直接相关的制度或文档\",\"generic-done\":\"已完成针对{query}的知识库检索\"}', '初始种子', 'agent');
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.steps.generate', 1, 'published', NULL,
 '{\"label\":\"生成回答\",\"before\":\"为{query}撰写回复\",\"active\":\"正在撰写并输出针对{query}的回复\",\"after\":\"已完成对{query}的回复\"}', '初始种子', 'agent');
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.steps.node', 1, 'published', NULL,
 '{\"before\":\"准备{displayName}\",\"active\":\"正在{displayName}\",\"after\":\"{displayName}完成\",\"before-with-query\":\"准备处理{query}的「{displayName}」环节\"}', '初始种子', 'agent');
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.steps.rag', 1, 'published', NULL,
 '{\"label\":\"检索知识库\",\"before\":\"在企业知识库中查找与{query}相关的资料\",\"active\":\"正在匹配与{query}最相关的文档片段\"}', '初始种子', 'agent');
...
INSERT IGNORE INTO prompt_version (prompt_id, version, status, content_text, content_json, change_note, maintainer) VALUES ('timeline.steps.think', 1, 'published', NULL,
 '{\"label\":\"深度思考\",\"before\":\"规划如何回答{query}\",\"active\":\"正在规划{query}的工具调用方案\",\"after\":\"已完成{query}的工具调用规划\",\"before-fallback\":\"规划工具与作答路径\",\"active-fallback\":\"正在规划工具调用方案\",\"after-fallback\":\"工具调用方案已拟定\",\"before-follow-up\":\"准备结合{toolDisplayName}结果继续分析\",\"active-follow-up\":\"正在综合分析{toolDisplayName}返回结果\",\"after-follow-up\":\"已完成{toolDisplayName}的工具结果综合分析\",\"before-follow-up-no-tool\":\"准备结合工具结果分析{query}\",\"active-follow-up-no-tool\":\"正在结合工具返回结果分析{query}\",\"after-follow-up-no-tool\":\"工具结果综合分析已完成\",\"before-follow-up-fallback\":\"准备结合工具结果分析\",\"active-follow-up-fallback\":\"正在综合分析工具结果\",\"after-follow-up-fallback\":\"工具结果分析完成\"}', '初始种子', 'agent');
