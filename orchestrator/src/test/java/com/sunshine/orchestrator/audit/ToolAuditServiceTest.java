...
class ToolAuditServiceTest {
    ...
    void toolCall_publishesScrubbedParams() {
        ...
        service.toolCall(
                ...
                "sdk__sunshine-finance__list_my_expenses", Map.of("status", "pending"), "共 3 条", "ok");
        ArgumentCaptor<AuditEvent> captor = ArgumentCaptor.forClass(AuditEvent.class);
        verify(auditPublisher).publish(captor.capture());
        assertThat(captor.getValue().eventType()).isEqualTo("tool.call");
        assertThat(captor.getValue().payloadJson()).contains("\"toolId\":\"sdk__sunshine-finance__list_my_expenses\"");
