...
WORKFLOWS: list[dict[str, Any]] = [
    {
        "id": "finance-list",
        "displayName": "我的报销查询",
        "description": "列出当前用户报销单（可按 status），不做合规分析",
        "mode": "workflow",
        "plan": {
            "planId": None,
            "reason": "导入自 sunshine-workflows.yaml · finance-list",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
    ...
    },
    {
        "id": "finance-smart",
        "displayName": "报销智能分析",
        "description": "对照费用制度只读分析当前用户报销是否合规（含 Agent；不可提交报销）",
        "mode": "workflow",
        "plan": {
            "planId": None,
            "reason": "企业化只读报销分析：rag→list→agent(finance-analysis，无 submit)",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
                ...
                },
                {
                    ...
                    "displayName": "费用制度检索",
                    "params": {
                        "query": "费用差旅网约车报销：{{start.userQuery}}",
                        "topK": "3",
                        "retry.maxAttempts": "1",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "continue",
                ...
                },
                {
                    ...
                    "params": {
                        ...
                        "context": "制度检索：{{rag-fsmart1.output}}\n报销列表：{{tool-d4e8f901.output}}",
                        "skill": "finance-analysis",
                        "tools": T_LIST_EXP + "," + T_GET_EXP,
                        "maxIters": "4",
                        "systemOverlay": "本节点仅输出内部只读分析结论，不面向用户；禁止调用白名单外工具；禁止提交报销；禁止虚构金额。",
                        "retry.maxAttempts": "1",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "continue",
                    },
    ...
    },
    {
        "id": "finance-summary",
        "displayName": "报销汇总统计",
        "description": "按状态汇总当前用户报销单条数与金额",
        "mode": "workflow",
        "plan": {
            "planId": None,
            "reason": "导入自 sunshine-workflows.yaml · finance-summary",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
    ...
    },
    {
        "id": "knowledge-qa",
        "displayName": "知识库问答",
        "description": "查企业制度/流程/规定（青松假、网约车上限、安全/IT/法务/行政/PMO 等）；非查个人单据列表或合规对比",
        "mode": "workflow",
        "plan": {
            "planId": None,
    ...
    },
    {
        "id": "knowledge-dual",
        "displayName": "双路知识检索",
        "description": "人事与费用制度并行检索后汇总（如青松假+网约车上限；4.7.2 join 标杆）",
        "mode": "workflow",
        "plan": {
            "planId": None,
            "reason": "4.7.2 并行双 RAG 标杆（BPMN parallel-gateway）",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
                ...
                },
                {
                    ...
                    "displayName": "回答",
                    "params": {
                        "prompt": "你是企业知识助手。综合下方「人事制度检索」与「费用制度检索」结果回答，不得编造。\n\n若两路均为空，回复：企业知识库中暂无相关规定。\n禁止暴露英文流程/工具内部名。\n\n人事制度检索：\n{{rag-a1b2c3d4.output}}\n\n费用制度检索：\n{{rag-e5f6a7b8.output}}",
                        "retry.maxAttempts": "2",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "fail_fast",
                    },
    ...
    },
    {
        ...
        "plan": {
            ...
            "nodes": [
                ...
                },
                {
                    ...
                    "params": {
                        "query": "{{start.userQuery}}",
                        "context": "检索：{{rag-l1o2o3p4.output}}\n待报销：{{tool-t1o2o3p4.output}}\n假期余额：{{tool-leave01.output}}",
                        "skill": "finance-analysis",
                        "tools": T_LIST_EXP,
                        "maxIters": "3",
                        "systemOverlay": "本节点仅输出内部分析结论；综合制度检索、待报销与假期余额，禁止编造。",
                        "retry.maxAttempts": "1",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "continue",
                    },
                },
                {
                    ...
                    "displayName": "回答",
                    "params": {
                        "prompt": "你是企业助手。根据下方循环分析结论回答，不得编造。\n\n若结论为空，回复：暂无可用结果。\n禁止暴露 knowledge-loop 等内部名称。\n\n循环分析：\n{{loop-a1b2c3d4.output}}",
                        "retry.maxAttempts": "2",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "fail_fast",
                    },
        ...
        "catalogMeta": {
            ...
            "intentAfter": "将按条件循环（检索→工具→智能体）流程处理",
        },
    },
    {
        "id": "sandbox-agent",
        "displayName": "工作区沙箱写文件",
        "description": "Workflow agent 节点默认沙箱：用沙箱工具在对话工作区写文件后汇总（4.5 SUB 默认沙箱标杆）",
        "mode": "workflow",
        "plan": {
            "planId": None,
            "reason": "4.5 SUB 默认沙箱标杆（start→agent→answer；复用对话级 /workspace）",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
                ...
                },
                {
                    ...
                    "params": {
                        "query": "{{start.userQuery}}",
                        "context": "请在对话沙箱 /workspace 完成用户请求的文件写入；沙箱六工具已注入，无需 Catalog 白名单。",
                        "maxIters": "4",
                        "systemOverlay": "本节点为 Workflow 子 Agent，仅输出内部结论供下游润色。\n必须使用 sandbox__write（新建）或 sandbox__edit（已存在）在 /workspace 落盘；过渡语后立刻 tool call。\n禁止编造「已写入」而未实际调用工具；完成后用简短中文说明路径与要点。",
                        "retry.maxAttempts": "1",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "continue",
                    },
        ...
        "catalogMeta": {
            "examples": [
                "在工作区写一份 hello.txt 内容为 hello sandbox",
                "用沙箱新建 /workspace/note.md",
            ],
            "nodeSummary": [
                "start",
                "agent",
    ...
    },
    {
        ...
        "plan": {
            "planId": None,
            "reason": "企业假期助手：rag→余额→请假列表→agent(可 submit_leave)",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
                ...
                },
                {
                    ...
                    "params": {
                        "query": "{{start.userQuery}}",
                        "context": "制度：{{rag-hrleave1.output}}\n余额：{{tool-hrbal01.output}}\n请假单：{{tool-hrlist01.output}}",
                        "skill": "policy-review",
                        "tools": T_LEAVE_SUB,
                        "maxIters": "4",
                        "systemOverlay": "仅当用户明确申请且假别/日期等参数齐全时调用提交请假；否则只输出只读结论；禁止编造余额或单据。",
                        "retry.maxAttempts": "1",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "continue",
                    },
    ...
    },
    {
        ...
        "displayName": "费用合规闭环",
        "description": "检索费用制度并列出我的报销；合规分析，明确提交时可 submit_expense（HITL）",
        "mode": "workflow",
        "plan": {
            "planId": None,
            "reason": "费用合规闭环：rag→list→agent(可 submit_expense)",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
                ...
                },
                {
                    ...
                    "params": {
                        "query": "{{start.userQuery}}",
                        "context": "制度：{{rag-expcomp1.output}}\n报销列表：{{tool-explist1.output}}",
                        "skill": "compliance-check",
                        "tools": T_LIST_EXP + "," + T_GET_EXP + "," + T_SUBMIT_EXP,
                        "maxIters": "4",
                        "systemOverlay": "仅当用户明确要求提交且金额/事由等参数齐全时调用提交报销；否则只输出合规只读结论；禁止虚构金额与单据。",
                        "retry.maxAttempts": "1",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "continue",
                    },
        ...
        "catalogMeta": {
            ...
            "intentAfter": "将按费用合规闭环流程处理",
        },
    },
    {
        "id": "oa-task-assist",
        "displayName": "OA 待办助手",
        "description": "列出我的 OA 待办；用户点名 taskId 时可审批（HITL）",
        "mode": "workflow",
        "plan": {
            "planId": None,
            "reason": "OA 待办助手：list→agent(可 approve)",
            "nodes": [
                {
                    "id": "start",
                    "type": "start",
                ...
                },
                {
                    ...
                    "params": {
                        "query": "{{start.userQuery}}",
                        "context": "待办列表：{{tool-oalist01.output}}",
                        "tools": T_OA_APPROVE,
                        "maxIters": "3",
                        "systemOverlay": "仅审批用户明确点名的 taskId；否则只列示/说明待办；禁止编造 taskId。",
                        "retry.maxAttempts": "1",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "continue",
                    },
                },
                {
                    ...
                    "displayName": "回答",
                    "params": {
                        "prompt": "根据上游工具与 Agent 结论回答用户。\n\n约束：禁止向用户暴露英文流程/工具内部名；仅依据下方数据与结论作答，禁止编造。\n\n待办列表：\n{{tool-oalist01.output}}\n\nAgent 结论：\n{{agent-oaassist1.answer}}",
                        "retry.maxAttempts": "2",
                        "retry.backoffMs": "500",
                        "retry.onFailure": "fail_fast",
                    },
